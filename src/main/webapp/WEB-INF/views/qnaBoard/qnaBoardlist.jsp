		...
		<div id="forCenter">
			...
			<table class="table">
				...
				<c:forEach items="${qbList }" var="qnaboard" varStatus="i">
					<tr>
						<td>${i.count }</td>
						<td><c:if test="${qnaboard.qnaType == 1 }">상품문의</c:if> <c:if test="${qnaboard.qnaType == 2 }">배송문의</c:if> <c:if test="${qnaboard.qnaType == 3 }">교환/환불</c:if> <c:if test="${qnaboard.qnaType == 4 }">기타문의</c:if></td>
						...
						<c:choose>
							<c:when test="${not empty qnaboard.qnaPassword and member.memberNo != '2'}">
								<td onclick='lock(this, "${qnaboard.qnaNo }")'><c:if test="${qnaboard.perfumeNo ne 0}">* </c:if>${qnaboard.qnaSubject}<i class="fa-solid fa-lock" style="margin-left: 5px;"></i></td>
							</c:when>
							<c:otherwise>
								
								<!-- 꼭 필요!!! -->
								<c:if test="${qnaboard.perfumeNo ne 0 }">
									<td><a href="/qnaboard/qnaDetail/${qnaboard.qnaNo}"><c:if test="${qnaboard.perfumeNo ne 0}">* </c:if>${qnaboard.qnaSubject}</a></td>
								</c:if>
								<c:if test="${qnaboard.perfumeNo eq 0 }">
									<td><a href="/qnaboard/detail?qnaNo=${qnaboard.qnaNo}"><c:if test="${qnaboard.perfumeNo ne 0}">* </c:if>${qnaboard.qnaSubject}</a></td>
								</c:if>
								<!-- 꼭 필요!!! -->
						...
						<td><c:if test="${qnaboard.replyStatus == 'Y'}">답변완료</c:if> <c:if test="${qnaboard.replyStatus != 'Y'}">답변대기</c:if></td>
					</tr>
					<c:choose>
						<c:when test="${not empty qnaboard.qnaPassword}">
							<tr style="display: none;" class="hiddenTr">
								...
								<!-- 							<td ></td> -->
								<td>
									<form action="/qnaboard/pwdetail" method="get" onsubmit="return validatePassword(${qnaboard.qnaNo})">
										<div class="hidden-password">
											비밀번호 : <input type="password" name="qnaPassword" id="qnaPassword${qnaboard.qnaNo}" class="hidden"> <input type="hidden" name="qnaNo" id="qnaNo${qnaboard.qnaNo}" value="${qnaboard.qnaNo}">
											<!-- 꼭 필요!! -->
				...
				<tr>
						...
						<div id="paging">
							<c:if test="${paging.totalCount ne null }">
								<c:if test="${paging.currentPage != 1}">
									...
									<!-- 이전 페이지로 버튼 -->
									<a href="/qnaboard/list?page=${paging.currentPage-1}" class="move prev">&lt;</a>
								</c:if>

								<c:forEach begin="${paging.startNavi}" end="${paging.endNavi}" var="i">
									<c:choose>
										<c:when test="${i == paging.currentPage}">
											<span class="page current" style="color: white; background-color: #222222; border-radius: 25%;">${i}</span>
										</c:when>
										<c:otherwise>
								...
								<c:if test="${paging.currentPage != paging.lastPage}">
									<!-- 다음 페이지로 버튼 -->
									<a href="/qnaboard/list?page=${paging.currentPage+1}" class="move next">&gt;</a>
									<c:if test="${paging.endNavi != paging.lastPage}">
										<!-- 맨 끝 페이지로 버튼 -->
										<a href="/qnaboard/list?page=${paging.lastPage}" class="move last">&gt;&gt;</a>
									</c:if>
								</c:if>
							</c:if>
						</div>
					...
					<c:if test="${member.memberId != 'admin' }">
						<td><a href="/qnaboard/write"><input class="submit-btn" type="submit" value="문의 글 쓰기"></a></td>
					</c:if>
				</tr>
			</table>

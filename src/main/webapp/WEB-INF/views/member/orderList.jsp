    ...
    <body>
                    ...
                    <tbody>
                        <c:forEach items="${orderList}" var="order" varStatus="i">
                            <tr class="orderTrTag">
                                ...
                                <td id="perfumeName${i.index}"><a href="/perfume/detail/${order.perfumeNo}">${order.perfumeName}</a></td>
                                <td id="perfumePrice${i.index}"><fmt:formatNumber value="${order.perfumePrice}" pattern="#,##0"/></td>
                                <td id="orderQuantity${i.index}">${order.orderQuantity}</td>
                                <td id="paymentAmount${i.index}"><fmt:formatNumber value="${order.paymentAmount}" pattern="#,##0"/></td>
                                <td id="orderStatus${i.index}">${order.orderStatus}</td>
                                <td id="reviewStatus${i.index}">
                                    <c:choose>
										<c:when test="${order.reviewStatus eq 0}">
                                            <c:choose>
                                                <c:when test="${order.orderStatus eq '배송완료'}">
                                                    <button onclick = "location.href = '/review/reviewWrite?orderInfo=${order.memberNo};${order.perfumeNo};${order.orderNo}'">후기작성</button>
                                                </c:when>
                                                <c:otherwise>
                                                    <button style="background-color: rgb(175, 175, 175); cursor: unset;">후기작성</button>
                                                </c:otherwise>
                                            </c:choose>
										</c:when>
										<c:otherwise>
        ...
        <script>
            ...
            tableFunc = function() {
                let prevOrderNo = 0;
                let orderNoCnt = 1;
                $('.orderTrTag').each(function(index, element) {
                    if (prevOrderNo == $(element).find('[id^=orderNo]').html()) {
                        orderNoCnt += 1;
                    } else {
                        if (prevOrderNo != 0 && orderNoCnt != 1) {
                            $('#orderNo' + (index - orderNoCnt)).attr('rowspan', orderNoCnt);
                            $('#paymentAmount' + (index - orderNoCnt)).attr('rowspan', orderNoCnt);
                            $('#orderStatus' + (index - orderNoCnt)).attr('rowspan', orderNoCnt);
                    ...
                    if (index ==  $('.orderTrTag').length - 1 && orderNoCnt != 1) {

...
public class AdMemberController {
	...
	public ModelAndView amModify(
		...
		try {
			...
			if(result > 0) {
				Alert alert = new Alert("/member/amList", "정보변경 성공했습니다.");
				mv.addObject("alert", alert);
				mv.setViewName("common/alert");
			} else {
				Alert alert = new Alert("/member/amList", "정보변경 실패했습니다.");
				mv.addObject("alert", alert);
				mv.setViewName("common/alert");
			}
		} catch (Exception e) {
	...
	public ModelAndView memberDetailView(
		...
		try {
			...
			mv.setViewName("member/amMypage");
		} catch (Exception e) {
			e.printStackTrace();
